...
const EnterPin = () => {
  ...
  const sendOtp = (e) => {
    ...
    if (!otpRef.current.value) {
      ...
      } else if (servedOtp?.email) {
        data = {
          hash: servedOtp.hash,
          email: servedOtp?.email,
          otp: otpRef.current.value,
        };
      }
      sendOTP(data)
  ...
  return (
    <section className="enterpin container section__card">
      <Card logo="lock" text="Enter the Code We Just Sent You">
        ...
        <p className="card__message">
          By entering the code, you’re agreeing to our Terms of Service and
